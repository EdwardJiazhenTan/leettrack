...
const mockedCreateQuestion = createQuestion as jest.MockedFunction<typeof createQuestion>;
const mockedSearchQuestions = searchQuestions as jest.MockedFunction<typeof searchQuestions>;
...
describe('/api/questions', () => {
  ...
  describe('GET', () => {
    ...
    it('should return questions list with search parameters', async () => {
      ...
      const request = new Request('http://localhost:3000/api/questions?query=array&difficulty=Easy&tags=Array,Hash Table&limit=10&offset=5&sort_by=title&sort_order=asc');
    ...
    it('should return 400 error when limit exceeds maximum', async () => {
      // Arrange
      const request = new Request('http://localhost:3000/api/questions?limit=150');
    ...
    it('should handle is_custom parameter correctly', async () => {
      ...
      const request = new Request('http://localhost:3000/api/questions?is_custom=true');
    ...
    it('should return 500 error when an exception occurs', async () => {
      ...
      expect(responseData.status).toBe('error');
      expect(responseData.message).toBe('An error occurred while fetching questions');
    });
  });
  ...
  describe('POST', () => {
    it('should create a new question successfully', async () => {
      ...
      expect(mockedCheckSlugExists).toHaveBeenCalledWith(mockCreateQuestionRequest.slug);
      expect(mockedCreateQuestion).toHaveBeenCalledWith(mockCreateQuestionRequest, 'user_123');
    });
    ...
    it('should return 400 error when required fields are missing', async () => {
      ...
      expect(response.status).toBe(400);
      expect(responseData.status).toBe('error');
      expect(responseData.message).toBe('Title, slug, and difficulty are required');
    });
    ...
    it('should return 500 error when an exception occurs', async () => {
      ...
      expect(response.status).toBe(500);
      expect(responseData.status).toBe('error');
      expect(responseData.message).toBe('An error occurred while creating the question');
    });
  });
});