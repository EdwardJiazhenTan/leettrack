...
class UserLearningPath(db.Model):
    ...
    question_progress = db.relationship(
        "UserPathQuestion", back_populates="user_path", cascade="all, delete-orphan"
    )

    __table_args__ = (db.UniqueConstraint("user_id", "path_id", name="uq_user_path"),)
