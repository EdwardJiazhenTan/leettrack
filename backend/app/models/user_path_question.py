...
class UserPathQuestion(db.Model):
    ...
    user_path_id = db.Column(
        db.Integer, db.ForeignKey("user_learning_paths.user_path_id"), nullable=False
    )
    path_question_id = db.Column(
        db.Integer, db.ForeignKey("path_questions.path_question_id"), nullable=False
    )
    ...
    path_question = db.relationship(
        "PathQuestion", back_populates="user_path_questions"
    )

    __table_args__ = (
        db.UniqueConstraint(
            "user_path_id", "path_question_id", name="uq_user_path_question"
        ),
    )
