            ...
            "questions": [
                {
                    ...
                    "last_updated": (
                        q.last_updated.isoformat() if q.last_updated else None
                    ),
                }
                for q in questions
            ],
            "user_questions": [
                {
                    ...
                    "last_reviewed": (
                        uq.last_reviewed.isoformat() if uq.last_reviewed else None
                    ),
                    "next_review": (
                        uq.next_review.isoformat() if uq.next_review else None
                    ),
            ...
            "review_schedules": [
                {
                    ...
                    "user_id": rs.user_id,
                    "review_date": (
                        rs.review_date.isoformat() if rs.review_date else None
                    ),
            ...
            "scheduled_reviews": [
                {
                    ...
                    "reviewed_at": (
                        sr.reviewed_at.isoformat() if sr.reviewed_at else None
                    ),
            ...
            "user_learning_paths": [
                {
                    ...
                    "path_id": ulp.path_id,
                    "started_at": (
                        ulp.started_at.isoformat() if ulp.started_at else None
                    ),
                    "completed_at": (
                        ulp.completed_at.isoformat() if ulp.completed_at else None
                    ),
            ...
            "user_path_questions": [
                {
                    ...
                    "completed_at": (
                        upq.completed_at.isoformat() if upq.completed_at else None
                    ),
                }
    ...
    try:
        ...
        for table_name, model in models:
            try:
                ...
                tables_info.append({"table_name": table_name, "row_count": count})
            except Exception as e:
                logger.error(f"Error counting {table_name}: {str(e)}")
                tables_info.append(
                    {"table_name": table_name, "row_count": 0, "error": str(e)}
                )
