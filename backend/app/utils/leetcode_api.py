...
class LeetCodeAPI:
    ...
    def _ensure_initialized(self):
        ...
        if self.base_url is None:
            from flask import current_app
            self.base_url = current_app.config.get('LEETCODE_API_BASE_URL', 'https://alfa-leetcode-api.onrender.com')
    ...
    def _handle_rate_limit(self, response):
        ...
        if response.status_code == 429:
            ...
            if reset_time:
                self.rate_limit_reset = datetime.now() + timedelta(seconds=int(reset_time))
                logger.warning(f"Rate limit hit. Reset at: {self.rate_limit_reset}")
            
            return {
                'error': 'Rate limit exceeded',
                'message': f'Too many requests. Try again in {reset_time} seconds' if reset_time else 'Rate limit exceeded',
    ...
    def _make_request(self, endpoint, params=None):
        ...
        if self.rate_limit_reset and datetime.now() < self.rate_limit_reset:
            ...
            logger.warning(f"Still in rate limit period. {remaining_time} seconds remaining")
            return {
        ...
        try:
            ...
            return {'error': 'Connection Error', 'message': 'Unable to connect to LeetCode API'}
        except requests.exceptions.RequestException as e:
            logger.error(f"Request error: {str(e)}")
            return {'error': 'Request Error', 'message': str(e)}
        except json.JSONDecodeError:
            logger.error("Invalid JSON response")
            return {'error': 'Invalid Response', 'message': 'Invalid JSON response from API'}
    ...
    def get_user_yearly_calendar(self, username, year):
        ...
        return self._make_request(f"/userProfileCalendar?username={username}&year={year}")
