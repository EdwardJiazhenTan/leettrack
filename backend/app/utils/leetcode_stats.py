...
def extract_recent_submissions(
    profile_data: Dict[str, Any], limit: int = 10
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for submission in recent_submissions[:limit]:
            ...
            submission_date = datetime.fromtimestamp(timestamp).strftime(
                "%Y-%m-%d %H:%M"
            )
    ...
    try:
        ...
        for timestamp_str, count in calendar_data.items():
            ...
            daily_submissions.append(
                {"date": date, "count": count, "timestamp": timestamp}
            )
