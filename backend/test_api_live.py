...
def test_health_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            success = data.get('name') == 'LeetTrack API' and data.get('status') == 'active'
            print_test_result("Root endpoint (/)", success, f"Status: {response.status_code}", response_time)
        else:
            print_test_result("Root endpoint (/)", False, f"Status: {response.status_code}", response_time)
    ...
    try:
        start_time = datetime.now()
        response = requests.get(f"{API_BASE_URL.replace('/api/v1', '')}/ping", timeout=10)
        response_time = (datetime.now() - start_time).total_seconds()
        
        if response.status_code == 200:
            data = response.json()
            success = data.get('message') == 'pong'
            print_test_result("Ping endpoint (/ping)", success, f"Status: {response.status_code}", response_time)
        else:
            print_test_result("Ping endpoint (/ping)", False, f"Status: {response.status_code}", response_time)
    except Exception as e:
...
def test_leetcode_endpoints():
    ...
    try:
        ...
        print_test_result("Get daily question", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
    ...
    try:
        ...
        print_test_result("Get problems list", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
    ...
    try:
        ...
        print_test_result("Get specific problem", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
...
def test_learning_paths_endpoints():
    ...
    try:
        ...
        print_test_result("Get learning paths", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
    ...
    try:
        start_time = datetime.now()
        response = requests.get(f"{API_BASE_URL}/learning-paths?difficulty=Easy&limit=3", timeout=10)
        response_time = (datetime.now() - start_time).total_seconds()
        
        success = response.status_code == 200
        print_test_result("Get filtered learning paths", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
...
def test_auth_endpoints():
    ...
    try:
        ...
        print_test_result("Register endpoint validation", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
    ...
    try:
        ...
        print_test_result("Login endpoint validation", success, f"Status: {response.status_code}", response_time)
    except Exception as e:
...
def test_protected_endpoints():
    ...
    for endpoint in protected_endpoints:
        try:
            ...
            print_test_result(f"Protected endpoint {endpoint}", success, f"Status: {response.status_code}", response_time)
        except Exception as e:
            print_test_result(f"Protected endpoint {endpoint}", False, f"Error: {str(e)}")
...
def test_cors_headers():
    ...
    try:
        ...
        print_test_result("CORS headers present", cors_configured, 
...
def main():
    ...
    print(f"{Colors.YELLOW}This is normal and doesn't indicate problems with your API.{Colors.ENDC}")
